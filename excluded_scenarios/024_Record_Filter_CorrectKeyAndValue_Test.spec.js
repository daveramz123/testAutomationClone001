...
let poManager2;
let accesstoken;
test.describe.serial(`Record Filter Correct Key and Value Tests`, async () => {
    ...
    test('Verify New User Creation', async ({}) => {
        ...
        await new Promise(resolve => setTimeout(resolve, 3000)); // 3 sec
        await poManager.UsersPage.AddGroup(dataSet.QAGroupRecordWithCorrectKeyAndValue);
        await new Promise(resolve => setTimeout(resolve, 3000)); // 3 sec
    ...
    test('Test Filter Record with Both Correct Key and Value', async ({request}) => {
        ...
        await expect(respBody.includes("expectedResult")).toBe(true);
